...
class SharePointAPI
{
     ...
     */
    public function __construct($spUsername, $spPassword, $spWsdl, $mode = 'STANDARD', $options = [])
    {
     ...
     */
    final public function __call($methodName, array $methodParams)
    {
        /*
        ...
        if (substr($methodName, 0, 2) == '__') {
            ...
            $returned = call_user_func_array([$this->soapClient, $methodName], $methodParams);
        } else {
            // Call it
     ...
     */
    public function getLimitedLists(array $keys, array $params = ['hidden' => 'False'], $isSensetive = true)
    {
    ...
    public function getLists()
    {
        ...
        if (! isset($results)) {
            $results = ['warning' => 'No data returned.'];
        }
    ...
    {
        return $this->writeMultiple($list_name, [$data]);
    }
    ...
    {
        return $this->writeMultipleToFolder($list_name, $folderPath, [$data]);
    }

    // Alias (Identical to above)
    public function add($list_name, array $data)
    {
        return $this->write($list_name, $data);
    }
    public function insert($list_name, array $data)
    {
        return $this->write($list_name, $data);
    }
    ...
    // Alias (Identical to above)
    public function addMultiple($list_name, array $items)
    {
        return $this->writeMultiple($list_name, $items);
    }
    public function insertMultiple($list_name, array $items)
    {
        return $this->writeMultiple($list_name, $items);
    }
    ...
    public function update($list_name, $ID, array $data)
    {
        ...
        return $this->updateMultiple($list_name, [$data]);
    }

    // aliases
    public function edit($list_name, $ID, array $data)
    {
        return $this->update($list_name, $ID, $data);
    }
    ...
    // aliases
    public function editMultiple($list_name, array $items)
    {
        return $this->updateMultiple($list_name, $items);
    }
    ...
    {
        return $this->deleteMultiple($list_name, [$ID], [$ID => $data]);
    }
    ...
    public function prepBatch(array $items, $method)
    {
        ...
        assert(in_array($method, ['New', 'Update', 'Delete']));
    ...
    public function magicLookup($name, $list)
    {
        ...
        $find = $this->read($list, null, ['Title' => $name]);
    ...
    public function getColumnVersions($list, $id, $field)
    {
